...
def get_alpha_q(alpha,q,V=15):
    return math.atan2( (-q*x_t + V*math.sin(alpha)), (V*math.cos(alpha)) )
...
if __name__ == "__main__":
    ...
    # alphas = np.linspace(-45,45,100)
    # qs = np.linspace(1,60,3)
    # cmqs = np.zeros((len(qs),len(alphas)))
    # c_ltas = np.zeros((len(qs),len(alphas)))
    # alpha_qs = np.zeros((len(qs),len(alphas)))
    
    # for j,alpha in enumerate(alphas):
    #     for i,q in enumerate(qs):
    #         cmqs[i,j] = get_cmq(math.radians(alpha),math.radians(q))
    #         alpha_q = get_alpha_q(math.radians(alpha),math.radians(q))
    #         c_ltas[i,j] = c_lta(alpha_q) - c_lta(math.radians(alpha))
    #         alpha_qs[i,j] = alpha_q

    # for i,q in enumerate(qs):
    #     plt.plot(alphas,cmqs[i,:])
